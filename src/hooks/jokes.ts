...
import { deliverJokesApi, moderateJokesApi, submitJokesApi } from "@/services/api.service";
...
export const useGetJokesForModeration = () => {
  return useQuery<PendingJoke[]>(
    ['get-jokes-for-moderation'],
    async () => {
      const response = await moderateJokesApi.get('/api/jokes/moderate', getAuthConfig());
      return response.data;
    }
  );
};
...
export const useApproveJoke = () => {
  ...
  return useMutation(
    async (joke: { jokeId: string; content: string; type: string }) => {
      const response = await moderateJokesApi.post('/api/jokes/approve', joke, getAuthConfig());
      return response.data;
    },
    {
      onSuccess: () => {
...
export const useRejectJoke = () => {
  ...
  return useMutation(
    async (jokeId: string) => {
      const response = await moderateJokesApi.delete(`/api/jokes/reject/${jokeId}`, getAuthConfig());
      return response.data;
    },
    {
      onSuccess: () => {
